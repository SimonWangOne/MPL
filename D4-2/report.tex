...
最终我们得到傅里叶变换的表达式
\beq
c(k) = \int \frac{1}{2\pi} f(x)e^{-ikx}dx
\eeq
...
\subsubsection{计算代码}
我使用了python计算上述傅里叶变换和求解正弦波的频率，完整的数据与代码可以在我个人的github网站~\cite{git_xu}中找到。首先是进行傅里叶变换并画图的代码：
...
\beq
\frac{\delta B}{2B^{\prime}} = 0.02909
\eeq
通过与之前实验内容类似的方法计算得到$2B^{\prime} = 97.6 \mathrm{mT}$，进而计算得到
\beq
\delta B = 2.84 \mathrm{mT}
\eeq
前面的分析与讨论我们得到了氟原子核的磁旋比
\beq
\gamma_F = 0.258 \mathrm{MHz/T}
\eeq
所以我们得到估算的氟原子核在聚四氟乙烯样品中的弛豫时间
\beq
T_2 \simeq 1.36 \mathrm{ms}
\eeq
\subsubsection{计算代码}
\begin{lstlisting}
import matplotlib.pyplot as plt
